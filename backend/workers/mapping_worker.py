...
from backend.consequence_engine.importance_scorer import get_mapping_depth, plan_routing
from backend.workers.outcome_worker import current_calibrator
...
async def map_event(event: NarrativeEvent, db: AsyncSession, forced_depth: str | None = None, calibrator: dict | None = None) -> dict | None:
    depth = forced_depth or get_mapping_depth(
    ...
    pred = result.get("prediction_score")
    calibrated = round(100 * calibration.apply_isotonic(calibrator, pred / 100)) if (pred is not None and calibrator) else pred
    ...
    consequence_map = EventConsequenceMap(
        ...
        calibrated_prediction_score=calibrated,
...
async def run_mapping_worker() -> dict:
    ...
    async with AsyncSessionLocal() as db:
        result = await db.execute(
            select(NarrativeEvent.id, NarrativeEvent.global_importance_score)
        ...
        event_ids = [row[0] for row in candidates]
        calibrator = await current_calibrator(db)  # isotonic recalibration from past outcomes

    routing_plan = plan_routing(
        [(row[0], row[1] or 0.0) for row in candidates],
        settings.importance_threshold_deep,
        settings.importance_threshold_light,
        settings.max_deep_mappings_per_run,
    )
        ...
        try:
            async with AsyncSessionLocal() as db:
                ...
                if not event or event.is_mapped:
                    continue
                meta = await map_event(event, db, forced_depth=routing_plan.get(event_id), calibrator=calibrator)
                await db.commit()

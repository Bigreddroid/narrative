# Narrative — Run the Beta Locally (Windows)

This guide gets the Narrative beta running on Windows (PowerShell). There are two ways:
...
```powershell
# Node.js LTS (20+) and Git — needed for both paths
...
Open `web\.env` in a text editor and set these two lines:
...
> Shortcut: `START_DEMO.bat` in the repo root can launch the frontend for you once
> `npm install` has been run — double-click it instead of step A2.

---

## Path B — Full local stack (offline / development)

This runs everything on your PC and loads a sample database so the app isn't empty.
...
Load the sample data (~8 MB dump of real events). Run it via `cmd` so the input
redirection works cleanly:

```powershell
cmd /c "docker compose exec -T postgres pg_restore -U narrative -d narrative --no-owner --clean --if-exists < scripts\narrative.dump"
```
...
```powershell
cd web
Copy-Item .env.example .env    # keep VITE_API_TARGET blank, VITE_DEMO_MODE=false
npm install
npm run dev
```
...
- **`npm` / `git` not recognized** — reopen PowerShell (or sign out/in) after installing
  so the PATH updates.

...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-paper text-ink font-sans selection:bg-crimson selection:text-paper">
      ...
      {/* ── Nav — dark terminal to match new hero + mobile reference */}
      <nav className="bg-[#0D1117] border-b border-white/10 px-6 md:px-10 py-4 flex justify-between items-center text-[#E8E8E8]">
        <div className="flex items-center gap-3">
          ...
          <span className="font-display text-xl tracking-wide">THE NARRATIVE</span>
        </div>
        <div className="flex items-center gap-6">
          <button
            onClick={() => document.getElementById("pricing")?.scrollIntoView({ behavior: "smooth" })}
            className="text-[11px] font-semibold uppercase tracking-widest text-white/40 hover:text-[#C80028] transition-colors hidden sm:block"
          >
            Pricing
          </button>
          <button
            onClick={() => navigate(isLoggedIn ? "/world" : "/auth")}
            className="text-[11px] font-bold uppercase tracking-widest border border-white/20 px-5 py-2 hover:bg-white/5 hover:border-[#C80028] hover:text-[#C80028] transition-colors"
          >
      ...
      {/* ── Hero — now denser, darker, product-forward to match mobile reference feel */}
      <main className="bg-[#0D1117] text-[#E8E8E8] border-b border-white/10">
        <div className="max-w-5xl mx-auto px-6 md:px-10 pt-16 pb-12">
          <div className="flex flex-col items-center text-center">
            <div className="inline-flex items-center gap-2 border border-white/15 px-4 py-1.5 mb-8 text-[10px] font-semibold uppercase tracking-[0.2em] text-white/50">
              <span className="w-1.5 h-1.5 rounded-full bg-[#C80028] animate-pulse" />
              LIVE INTELLIGENCE · 100+ SOURCES
            </div>

            <h1 className="font-display text-5xl md:text-6xl lg:text-7xl leading-none tracking-[-1.5px] mb-4">
              THE WORLD IN <span className="text-[#C80028]">CONFLICT</span><br />
              CONSEQUENCE MAPPED.
            </h1>

            <p className="max-w-lg text-[15px] text-white/60 mb-8">
              Every global event traced through cause and mechanism to its real-world consequence.
              Dense intelligence. Not headlines.
            </p>

            <div className="flex flex-col sm:flex-row items-center gap-3">
              <button
                onClick={() => navigate(isLoggedIn ? "/world" : "/auth")}
                className="px-7 py-3 bg-[#C80028] text-white text-[12px] font-bold uppercase tracking-widest rounded hover:bg-[#E0002E] transition-colors"
              >
                OPEN THE MAP →
              </button>
              <button
                onClick={() => document.getElementById("how")?.scrollIntoView({ behavior: "smooth" })}
                className="px-7 py-3 border border-white/20 text-[12px] font-bold uppercase tracking-widest text-white/70 hover:text-white hover:border-white/40 transition-colors"
              >
                SEE HOW IT WORKS
              </button>
            </div>

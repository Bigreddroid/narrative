...
import { useTrafficNearEvent } from "../hooks/useTrafficNearEvent.js";
...
export default function EventDetail() {
  ...
  const { can }      = useUser();
  const { event: rawEvent, revisions, loading, error } = useEventGraph(eventId);
  const [activeTab, setActiveTab] = useState("Intelligence");

  const following = isFollowing(eventId);

  // Real-only: never fabricate event detail. If the real event can't load,
  // `event` stays null and the UI shows an honest error state below.
  ...
  const traffic = useTrafficNearEvent(event);
    ...
    return (
      <div className="flex flex-col h-screen overflow-hidden bg-paper">
        ...
        <div className="flex-1 flex items-center justify-center px-6">
          <p className="text-ink/40 text-sm font-mono text-center">
            Couldn't load this event from the live feed.<br />Please check your connection and try again.
          </p>
        </div>
      </div>
    );
  ...
  // Real trajectory: the event's prediction-score history across its revisions
  // (oldest→newest, from /events/{id}/revisions). Rendered ONLY with ≥2 real
  // points — never a fabricated line. Historical analogs match against real past
  // events (lib/analogs.js), a curated reference set.
  const trajVals  = (revisions || []).map((r) => r.prediction_score).filter((v) => typeof v === "number");
  const hasTraj   = trajVals.length >= 2;
  const trajTrend = hasTraj ? trendLabel(momentum(trajVals)) : "stable";
  const trajPts   = hasTraj
    ? trajVals.map((v, i) => `${(i / (trajVals.length - 1)) * 80},${24 - (Math.max(0, Math.min(100, v)) / 100) * 24}`).join(" ")
    : "";
  const analogs   = findAnalogs(event, HISTORICAL_ANALOGS, 2);
        ...
        <div className="max-w-[720px] mx-auto px-4 sm:px-6 pt-6 pb-28 md:pb-10">
          ...
          >
            ...
            {(traffic.vessels + traffic.aircraft) > 0 && (
              <p className="mt-3 text-[10px] font-mono uppercase tracking-wider text-ink/35">
                <span style={{ color: "#3FA7A0" }}>{traffic.vessels} vessels</span>
                {" · "}
                <span style={{ color: "#5BA3D0" }}>{traffic.aircraft} flights</span>
                {" in the affected zone"}
              </p>
            )}

            {/* Trajectory — real prediction-score history (≥2 revisions only) */}
            {hasTraj && (
              <div className="mt-4 flex items-center gap-2">
                <span className="text-[9px] font-mono uppercase tracking-wider text-ink/35">Trajectory</span>
                <svg width="80" height="24"><polyline points={trajPts} fill="none" stroke={color} strokeWidth="1.3" strokeLinejoin="round" /></svg>
                <span className="text-[10px] font-mono" style={{ color: trajTrend === "rising" ? "#C80028" : trajTrend === "falling" ? "#2E8B57" : "rgba(26,26,26,0.4)" }}>
                  {trajTrend === "rising" ? "↑ escalating" : trajTrend === "falling" ? "↓ easing" : "→ stable"}
                </span>
              </div>
            )}

            {/* Historical analogs (pattern match) */}
            {analogs.length > 0 && (
              <div className="mt-3 border-t border-ink/8 pt-3">
                <p className="text-[9px] font-mono uppercase tracking-wider text-ink/35 mb-2">Historical analogs</p>
                <div className="space-y-1.5">
                  {analogs.map((a) => (
                    <div key={a.event.id} className="text-[12px] text-ink/60 leading-snug">
                      <span className="font-semibold text-ink/75">{a.event.title}</span>
                      <span className="text-ink/35"> · {a.event.date} · {a.similarity}% match · </span>
                      <span className="uppercase text-[10px] font-mono" style={{ color: a.event.outcome === "materialized" ? "#C80028" : "#C9A227" }}>{a.event.outcome}</span>
                      <span className="text-ink/45"> — {a.event.result}</span>
                    </div>
                  ))}
                </div>
              </div>

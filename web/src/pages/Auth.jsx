...
export default function Auth() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Authenticate via dev-login (works for both sign-in and sign-up)
      let res;
      try {
      ...
      if (!res.ok) {
        if (res.status >= 502 && res.status <= 504) throw new Error("Cannot reach the API — make sure the backend is running.");
